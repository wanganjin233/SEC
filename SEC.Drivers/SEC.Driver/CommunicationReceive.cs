...
{
    internal static class CommunicationReceive
    {
        /// <summary>
        /// 接收委托
        /// </summary>
        ...
        /// <returns></returns>
        public static List<byte[]>? ReceiveProcess(this ICommunicationInfo communicationInfo, System.IO.Ports.SerialPort serialPort, byte[] receiveBuffer)
        {
            byte[] receive()
            {
        ...
        private static List<byte[]>? ReceiveProcess(Receive receive, ICommunicationInfo communicationInfo, byte[] receiveBuffer)
        {
            try
            {
                ...
                while (true)
                {
                    byte[] buffer = receive(); //接收阻塞 
                    if (buffer == null) return null;//断开信号 
